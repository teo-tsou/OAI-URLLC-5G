...
int mhidevice_detect(char *qmichannel, char *usbnet_adapter, PROFILE_T *profile) {
    ...
    if((cntrl_idx = strstr(usbnet_adapter, "mhi"))) {
	if(atoi(cntrl_idx + 3) > 0)
	    snprintf(devname, sizeof(devname), "/dev/mhi_MBIM%s", cntrl_idx + 3);
        else
	    snprintf(devname, sizeof(devname), "/dev/mhi_MBIM");

	if (!access(devname, F_OK)) {
            strcpy(qmichannel, devname);
            profile->software_interface = SOFTWARE_MBIM;
            return 1;
        }

	if(atoi(cntrl_idx + 3) > 0)
	    snprintf(devname, sizeof(devname), "/dev/mhi_QMI0%s", cntrl_idx + 3);
        else
	    snprintf(devname, sizeof(devname), "/dev/mhi_QMI0");
